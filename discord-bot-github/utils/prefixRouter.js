		...
		switch (cmd) {
			...
			case 'addmoney': {
				if (!isAuthorized(message)) {
					await message.reply('⛔ Cuma admin server atau owner bot yang bisa pakai command ini.');
					return;
				}
				const target = message.mentions.users.first();
				const amount = parseAmount(firstNonMentionArg(args));
				if (!target || !amount) {
					await message.reply(`Format: \`${word} @user <jumlah>\` contoh: \`${word} @Budi 100k\` (boleh minus buat kurangi, mis. \`-50k\`)`);
					return;
				}
				if (target.bot) {
					await message.reply('Tidak bisa kasih Rp ke bot.');
				...
				await message.reply(`🛠️ ${verb} saldo **${target.username}** sebesar **${formatRupiah(Math.abs(amount))}**. Saldo sekarang: **${formatRupiah(newBalance)}**`);
				break;
			}
			...
			case 'help': {
				await message.reply(
					`**Command cepat (prefix \`${PREFIX}\`)**\n` +
						...
						`\`${ALIASES.addmoney.join('`/`')}\` @user <jumlah> — *[admin/owner]* gift saldo gratis\n\n` +
