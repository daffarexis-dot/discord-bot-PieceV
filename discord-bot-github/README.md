...
8. Buka tab **Variables** di project Railway itu, tambahkan 4 variable dari Langkah 1 tadi:
   ...
   Opsional (buat fitur command prefix `p...` dan `/addmoney`, lihat bagian bawah README):
   - `PREFIX` (default `p` kalau tidak diisi)
   - `OWNER_IDS` (User ID Discord kamu, boleh lebih dari satu dipisah koma)
...
| `/addmoney user:<@user> jumlah:<n>` | ⚙️ **[Admin/Owner saja]** Nambah (atau kurangi kalau minus) saldo user secara gratis, tanpa motong saldo siapapun. Lihat bagian "Command admin" di bawah. |
...
| `padd` / `paddmoney` / `pgift` | `/addmoney` (admin/owner saja) | `padd @user 100k` |
| `phelp` / `pmenu` | — | tampilkan daftar command prefix di atas |
...
## Command admin — `/addmoney` (gift saldo gratis)

Command ini buat admin server atau owner bot ngasih saldo Rp ke player tanpa perlu punya saldo sendiri dulu (beda dari `/give` yang motong saldo si pengirim).

Yang boleh pakai:
- **Siapapun dengan izin Administrator** di server itu (otomatis, tidak perlu setting tambahan).
- **Owner bot** — isi Discord User ID kamu di `.env` pada variable `OWNER_IDS` (boleh lebih dari satu, pisah pakai koma), supaya kamu tetap bisa pakai command ini walau di server tertentu kamu bukan admin. Cara lihat User ID: aktifkan Developer Mode di Discord (Settings → Advanced), klik kanan profil kamu → Copy User ID.

Cara pakai: `/addmoney user:@player jumlah:100000` (jumlah boleh minus buat mengurangi saldo, misal `-50000`). Versi prefix-nya: `padd @player 100k`.
