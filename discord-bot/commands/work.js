...
module.exports = {
	data: new SlashCommandBuilder().setName('work').setDescription('Kerja buat dapat Rp'),
	...
	async execute(interaction) {
		...
		const amount = (Math.floor(Math.random() * 41) + 10) * 1000; // Rp 10.000 - Rp 50.000
		setCooldown(guild.id, user.id, 'lastWork');
		const newBalance = addBalance(guild.id, user.id, amount);
		await interaction.reply(`💼 Kamu ${job} dan dapat **${formatRupiah(amount)}**! Saldo sekarang: **${formatRupiah(newBalance)}**`);
	},
};
