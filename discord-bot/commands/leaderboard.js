...
module.exports = {
	data: new SlashCommandBuilder().setName('leaderboard').setDescription('Lihat 10 saldo terbanyak di server ini'),
	...
	async execute(interaction) {
		...
		const lines = await Promise.all(
			top.map(async (entry, i) => {
				...
				return `${i + 1}. **${name}** — ${entry.balance} koin`;
			})
		);

		await interaction.reply(`🏆 **Leaderboard Koin**\n${lines.join('\n')}`);
	},
};
