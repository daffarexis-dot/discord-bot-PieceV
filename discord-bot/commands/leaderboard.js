...
const { getLeaderboard, getMessageStreak, getStreakBadge, formatRupiah } = require('../utils/economy');
	...
	async execute(interaction) {
		...
		const lines = await Promise.all(
			top.map(async (entry, i) => {
				...
				const badge = getStreakBadge(getMessageStreak(interaction.guild.id, entry.userId));
				const badgeTag = badge ? ` ${badge.emoji}` : '';
				return `${i + 1}. **${name}**${badgeTag} — ${formatRupiah(entry.balance)}`;
			})
		);

		await interaction.reply(`🏆 **Leaderboard Rp**\n${lines.join('\n')}\n\n_Badge dari streak ngobrol harian: 🔥3+ hari ⚡7+ 🌟14+ 👑30+ 💎60+_`);
	},
};
