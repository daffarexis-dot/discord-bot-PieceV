...
const ALIASES = {
	...
	mancing: ['pm', 'pmancing'],
	berburu: ['pburu', 'pberburu'],
	nambang: ['pnambang', 'ptambang'],
	give: ['pg', 'pgive', 'pkasih'],
	leaderboard: ['pl', 'plb', 'pleaderboard', 'ptop'],
	addmoney: ['paddmoney', 'padd', 'ptambahsaldo'],
	help: ['phelp', 'pmenu'],
};

// Aksi/roleplay (hug, lick, kill, punch, dll) ditambahin otomatis dari utils/actionsData.js,
// jadi tinggal nambah entry di sana kalau mau nambah aksi baru tanpa perlu edit file ini.
for (const [key, action] of Object.entries(ACTIONS)) {
	ALIASES[key] = action.aliases;
}
...
async function handleMessage(message) {
	...
	// Command aksi/roleplay (hug, lick, kill, punch, dll) ditangani generik di sini
	// (bukan lewat switch di bawah) biar tidak perlu nulis case satu-satu buat tiap aksi.
	if (ACTIONS[cmd]) {
		...
		try {
			await runAction(cmd, author, target, responder);
		} catch (err) {
			console.error('[Action] Error:', err);
			await message.reply('Terjadi error saat menjalankan aksi ini.').catch(() => {});
		}
		return;
	}
	...
	try {
		switch (cmd) {
			...
			case 'nambang': {
				...
				await runGrind(cmd, guild.id, author.id, responder);
				break;
			}

			case 'addmoney': {
				if (!isOwner(author.id)) {
					await message.reply('🚫 Command ini khusus owner bot.');
					return;
				}
				const target = message.mentions.users.first() || author;
				...
				if (!amount) {
					await message.reply(
						`Format: \`${word} [@user] <jumlah>\` contoh: \`${word} @Budi 500k\` (boleh minus buat kurangi)`
					);
					return;
				}
				...
				await message.reply(
					`👑 ${isAdd ? 'Menambahkan' : 'Mengurangi'} **${formatRupiah(Math.abs(amount))}** ${
						isAdd ? 'ke' : 'dari'
					} saldo **${target.username}**. Saldo sekarang: **${formatRupiah(newBalance)}**`
				);
				break;
			}
