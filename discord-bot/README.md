...
| `/daily` | Klaim Rp harian: **Rp 100.000**, naik **kelipatan x2 tiap 7 hari streak** berturut-turut (harus klaim tiap hari, jangan sampai lewat 48 jam atau streak reset). Dapat badge otomatis sesuai panjang streak. |
| `/work` | Kerja buat Rp 10.000 - Rp 50.000 (cooldown 30 menit) |
| `/give user:<@user> jumlah:<n>` | Kasih Rp ke user lain |
| `/leaderboard` | Top 10 saldo Rp terbanyak di server, badge streak tiap user juga ditampilkan |
| `/coinflip bet:<n> pilihan:<heads/tails>` | Tebak koin, menang x2 |
| `/slots bet:<n>` | Slot machine dengan animasi spin — reel-nya berhenti satu-satu (kiri → tengah → kanan) biar kelihatan bergerak (3 sama = jackpot, 2 sama = x1.5) |

## Badge streak harian

Tiap kali klaim `/daily`, streak-mu dihitung. Semakin panjang streak berturut-turut, semakin tinggi badge yang kamu dapat. Badge ini muncul otomatis di `/daily`, `/balance`, dan `/leaderboard`:

| Badge | Nama | Minimal streak |
|---|---|---|
| 🔥 | Streak Awal | 3 hari |
...
Kalau streak reset (lewat 48 jam tanpa klaim), badge juga otomatis hilang/turun sesuai streak yang tersisa.
